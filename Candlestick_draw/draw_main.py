...
def calculate_ma(day_count: int, data: pd.DataFrame):
    ...
    for i in range(len(data["close"])):
        ...
        for j in range(day_count):
            sum_total += float(data["close"][i - j])
...
def draw(df: pd.DataFrame, ts_name: str):
    ...
    # line = Line().add_xaxis(df["trade_date"].values.tolist()).add_yaxis('10 日均线',sma_10,is_smooth=True).set_global_opts(yaxis_opts=opts.AxisOpts(is_scale=True))
    # line.add('10 日均线', df['trade_date'], sma_10, is_fill=False, line_opacity=0.8, is_smooth=True)
    # line.add('30 日均线', df['trade_date'], sma_30, is_fill=False, line_opacity=0.8, is_smooth=True)
    # overlap_kline_line = kline.overlap(line)

    kline.render("a.html")
    # overlap_kline_line.render("c.html")
